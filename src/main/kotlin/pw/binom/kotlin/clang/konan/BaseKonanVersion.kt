...
class BaseKonanVersion(val kotlinVersion: VersionNumber) : KonanVersion {
    ...
    private fun requiredProp(key: String): String =
        prop(key) ?: throw GradleException("'$key' not found in $konanPropsFile")
    ...
    private fun resolveKonanPath(raw: String): File {
        ...
        while (current.startsWith("$")) {
            if (depth++ > 20) {
                throw GradleException("Too deep property resolution for \"$raw\" in $konanPropsFile")
            }
